...
| TAC policy without guessed GSMA data | Module mode preserves the baseline TAC captured from the modem at install (normally the physical module TAC on a fresh device; merely the current TAC on an upgrade). Phone mode is user-supplied and fails closed until `tac-list-phone.txt` contains TACs with documented authoritative provenance. TAC prefixes do not encode manufacturer/device class. |
